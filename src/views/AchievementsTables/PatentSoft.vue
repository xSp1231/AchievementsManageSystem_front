...
<script setup>
import {DeleteFilled,Document, EditPen,Delete, Plus, Search, UploadFilled, Refresh, Download,CircleClose} from '@element-plus/icons-vue'
...
const addInfo = () => {
  ...
  isAdd.value=true;
  dialogTitle.value="专利软著信息填报-请事先查看公告里面的填报要求(成果标题,图片上传时(建议一张)要多考虑,后续修改过程可能略微繁琐)"
}
...
//导出全部
const exportAll=()=>{
  window.location.href = "http://47.109.143.177:8080/PatentSoft/exportAll";
}
//批量导出
const exportPart=()=>{
...
const submitFile = () => {
  ...
  Data.append("achievementName", formData.name) //将标题也加进去
  Data.append("username", formData.username) //将成果的名字也加进去
  api.post('/PatentSoftPicture/uploadPictures', Data, {headers: {'Content-Type': 'multipart/form-data'}}).then(res => {
...
const getImages = () => {
  api.get("/PatentSoftPicture/picturesList/" + formData.username + "/" + formData.name) //根据用户名 成果名字 查询对应的图片
      .then(res => {
...
const deleteImg=(it)=>{
  ...
  api.post("/PatentSoftPicture/deleteImg",info).then(res=>{
