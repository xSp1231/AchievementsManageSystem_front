  ...
  <div class="userManage" style="width: 100%;height: 100%;background-color: #ffffff;position: relative;">
    ...
    <div style="margin-left: 2%">
      <div class="findarea" style="">
        ...
        <el-input clearable class="filter-item" v-model="queryInfo.rewardName" placeholder="证书名称(奖项名称)"
                  style="width: 150px;margin-right: 8px"></el-input>
        ...
        <el-button v-if="store.state.role==='admin'" type="primary" plain :icon="Download" style="margin-left: 10px" @click="exportAll()">导出全部奖励数据
        </el-button>
...
<!--            action="http://8.137.9.219:8080/Reward/importData"-->
        ...
        >
          <el-form :model="formData" style="width: 80%" ref="rulesForm" :rules="rules" :inline="true">
            ...
            <el-form-item label="填报结果状态" label-width="150" prop="status">
              <el-select   :disabled="isStudent" v-model="formData.status" placeholder="成果填报状态状态">
                <el-option label="审核" value="审核"/>
                <el-option label="接收" value="接收"/>
                <el-option label="拒绝" value="拒绝"/>
              </el-select>
...
const getAll = () => { //分页+条件查询
  if (role === "student") {//如果为学生  那么就发请求 获得他的username 之后再发送请求   //vuex
...
const addInfo = () => {
  ...
  isAdd.value=true;
  dialogTitle.value="奖励信息填报-请事先查看公告里面的填报要求(成果标题,图片上传时(建议一张)要多考虑,后续修改过程可能略微繁琐)"
}
...
//导出全部
const exportAll=()=>{
  window.location.href = "http://47.109.143.177:8080/Reward/exportAll";
}
//批量导出
const exportPart=()=>{
...
const submitFile = () => {
  ...
  Data.append("achievementName", formData.rewardName) //将标题也加进去
...
const getImages = () => {
  api.get("/RewardPicture/picturesList/" + formData.username + "/" + formData.rewardName) //根据用户名 成果名字 查询对应的图片
      .then(res => {
...
const deleteImg=(it)=>{
  ...
  api.post("/RewardPicture/deleteImg",info).then(res=>{
